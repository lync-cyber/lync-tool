...
- 快速检测必须只查询当前配置需要的 WinGet package ID/source，并用精确查询的官方退出码确定安装状态，不得解析表格或扫描整机应用来展示版本。升级检查、安装后回滚登记和卸载前防漂移需要版本证据时才读取结构化 export；export 缺失不能当成未安装。每个只读 WinGet 子进程必须有超时上限。PATH 命令只验证可执行能力，不能代替包身份。
