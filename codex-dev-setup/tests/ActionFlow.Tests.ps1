...
Assert-True ($actionsText -match "@\('list', '--id'.*'--upgrade-available'\)") '软件更新必须使用只读的 WinGet list 查询。'
...
Assert-True ($actionsText -match 'Test-WingetPackageInstalled' -and $actionsText -match '为避免误卸载') '只有确认由本次新增的软件包才应登记为可回滚安装。'

Write-Host '动作流程检查通过：18 项' -ForegroundColor Green
