...
3. `source_plan`：由 LLM 根据本地市场和当前可用性选择来源，写入 `data/source_plan.json`。高优先级来源必须先在用户可见、可人工接管的浏览器中直接尝试并记录状态。
...
6. `coverage`：运行 `validate_source_coverage.py` 和 `validate_collection_coverage.py`；搜索引擎无结果不算平台尝试，首屏有足够广告也不算发现阶段完成。
